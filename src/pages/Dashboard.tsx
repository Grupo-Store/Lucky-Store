...
import { useOrders, OrderStatus, ORDER_STATUS_COLORS, calcTotal, isOpenOrder } from '@/store/OrderStore';
...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    const totalRevenue = orders.reduce((s, o) => s + calcTotal(o), 0);
    const openOrders = orders.filter(o => isOpenOrder(o.status)).length;
    const deliveredOrders = orders.filter(o => o.status === 'Delivered').length;
    const statusDist = STATUS_KEYS.map(s => ({ name: s, value: orders.filter(o => o.status === s).length })).filter(d => d.value > 0);
    ...
    const monthlyRevenue = months.map((m, i) => ({
      ...
      pedidos: i < 4 ? Math.max(1, Math.round(orders.length * (0.5 + Math.random()))) : 0,
    }));
    ...
    return { totalRevenue, openOrders, deliveredOrders, statusDist, monthlyRevenue, quarterlyData, totalOrders: orders.length };
  }, [orders]);
